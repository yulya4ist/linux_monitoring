...
total_folders=$(sudo find "$dir_path" -type d | wc | awk '{print $1}')
echo "Total number of folders (including all nested ones) = $total_folders"

# Top 5 folders with largest size
echo "TOP 5 Folders of maximum size arranged in descending order (path and size):"
top_folders=($(sudo du -h --max-depth=1 "$dir_path" | sort -rh))
for i in {0..4}; do
echo "$((i+1)) - ${top_folders[$i]}"
done

# Total number of files
total_files=$(sudo find "$dir_path" -type f | wc | awk '{print $1}')
echo "Total number of files = $total_files"

# Number of different types of files
conf_files=$(sudo find "$dir_path" -type f -name ".conf" | wc | awk '{print $1}')
text_files=$(sudo find "$dir_path" -type f -name ".txt" | wc | awk '{print $1}')
exe_files=$(sudo find "$dir_path" -type f -executable | wc | awk '{print $1}')
log_files=$(sudo find "$dir_path" -type f -name ".log" | wc | awk '{print $1}')
archive_files=$(sudo find "$dir_path" -type f -name ".zip" -o -name ".tar" -o -name ".gz" | wc | awk '{print $1}')
sym_links=$(sudo find "$dir_path" -type l | wc | awk '{print $1}')
echo "Number of:"
...
top_files=$(sudo find "$1" -type f -printf '%s %p\n' | sort -rh | head -n 10)
...
exec_files=$(sudo find "$1" -type f -executable)
